...
def main(image_path):
    ...
    real_width_microns = 24.44
    real_height_microns = 24.75
    ...
    if max_length_info is not None:
        ...
        midpoint = tuple(((pt1 + pt2) // 2).astype(int))
        cv2.putText(img, f"Longest: {length:.3f}", midpoint, cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
    ...
    if min_length_info is not None:
        ...
        midpoint = tuple(((pt1 + pt2) // 2).astype(int))
        cv2.putText(img, f"Shortest: {length:.3f}", midpoint, cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
    ...
    output_image_path = "stacking_fault/rate_output_with_side_lengths_longest_shortest.jpg"
    cv2.imwrite(output_image_path, img)
    print(f"Output image saved to {output_image_path}")

if __name__ == "__main__":
    image_path = "first/27.jpeg" 
    main(image_path)
